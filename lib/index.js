...
var weekStart = moment().add(currentWeek, 'weeks').startOf('isoWeek').format();
var weekEnd = moment().add(currentWeek, 'weeks').endOf('isoWeek').format();
...
function calendarDayHtml(day, dayOfWeek) {
  ...
  if(day.length){
    for(var i = 0; i < day.length; i++){
      ...
      summaries +=
        `<p class="summary">${summary + ' <span class="summary-time">(' +
